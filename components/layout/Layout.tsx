...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isMapRoute = loc && (loc.pathname === '/map-dashboard' || loc.pathname.startsWith('/map'));
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className={`flex-1 flex flex-col overflow-y-hidden ${contentMarginClass}`}>
        ...
        <main className={`flex-1 overflow-x-auto overflow-y-auto bg-gray-100 ${isMapRoute ? 'p-0' : 'p-4 md:p-6 lg:p-8'} mb-0 pb-0 transition-all duration-200`}>
          {children}
        </main>
      </div>
    </div>
